...
import com.github.sarxos.webcam.Webcam;   // to access the camera
...
public class Client2 extends Application
{
    ...
    static Thread UDP_send_Voice_Thread_ = null;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        UDP_recv_Voice_Thread = new Thread( ()->
        {
            ...
            try
            {
                ...
                DatagramSocket UDP_Connection = new DatagramSocket( 3333  );
            ...
            } catch ( Exception ex ){ System.out.println(" LOG:  Speaker is not recieving the correct Signal "); speaker.flush();  speaker.close(); }
        });



        UDP_send_Voice_Thread_ = new Thread( ()->
        {
            try
            {
                AudioFormat format = new AudioFormat(48000.0f, 16, 2, true, true);
                ...
                DatagramSocket UDP_Connection = new DatagramSocket(6666);
                System.out.println(" Mic is ON ");

                byte[] sig = new byte[1];
                DatagramPacket receivePacket = new DatagramPacket( sig, sig.length);
                UDP_Connection.receive(receivePacket);
                System.out.println("client connected   " + receivePacket.getData()[0] );
                ...
                while (true)
                {
                    ...
                    DatagramPacket UDP_packet =
                       new DatagramPacket(buffer, bytesRead, receivePacket.getAddress(), receivePacket.getPort() );
        ...
        Thread UDPcameraThread = new Thread( ()->{
            try
            {
                DatagramSocket UDP_Connection = new DatagramSocket(4444);
                byte[] sig = new byte[1];
                DatagramPacket receive_test_Packet = new DatagramPacket( sig, sig.length);
                UDP_Connection.receive(receive_test_Packet);
                System.out.println("client connected   " + receive_test_Packet.getData()[0] );

                byte[] carry = new byte[60000];
                DatagramPacket UDP_packet_recv_img = new DatagramPacket(carry, carry.length);
            ...
            while (true)
            {
              try{
               ...
               byte[] imageBytes = byteArrayOutputStream.toByteArray();  System.out.println(imageBytes.length);

         DatagramPacket UDP_packet_snd_img =
                new DatagramPacket(imageBytes, imageBytes.length, receive_test_Packet.getAddress(), receive_test_Packet.getPort());
                UDP_Connection.send(UDP_packet_snd_img);


                    /** Recv Frame  **/
                UDP_Connection.receive(UDP_packet_recv_img);
                int bytesRead = UDP_packet_recv_img.getLength();
        ...
        Thread UDP_recv_image_Thread = new Thread( ()->{
            try
            {
                DatagramSocket UDP_Connection = new DatagramSocket(5555);
                byte[] carry = new byte[60000];
                DatagramPacket UDP_packet_recv_img = new DatagramPacket(carry, carry.length);
